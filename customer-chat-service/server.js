...
const customerSocketsMap = new Map();
const executiveSocketsMap = new Map();
const customerWaitingQueue = [];
const executiveAvailability = new Map();
const assignedExecutiveToCustomer = new Map();
const findAvailableExecutive = () => {
...
wss.on("connection", (ws) => {
                    ...
                    if (from === "cust") {
                        ...
                        if (customerWaitingQueue.length) {
                            ...
                            customerPayload?.userId &&
                                assignedExecutiveToCustomer.set(customerPayload?.userId, userId);
            ...
            case "CHAT":
                if (from === "cust") {
                    ...
                    if (executiveUserId) {
                        ...
                        const customerWaiting = customerWaitingQueue.find((customer) => customer.userId === userId);
                        if (customerWaiting) {
    ...
    ws.on("close", () => {
        ...
        for (let [userId, socket] of customerSocketsMap.entries()) {
            if (socket === ws)
                customerSocketsMap.delete(userId);
        }
